...
1. **Python Syntax Check**
   ...
   - Command: `cd "$(git rev-parse --show-toplevel)/app/server" && uv run python -m py_compile server.py main.py core/*.py`
...
2. **Backend Code Quality Check**
   - Preparation Command: None
   - Command: `cd "$(git rev-parse --show-toplevel)/app/server" && uv run ruff check .`
...
3. **All Backend Tests**
   - Preparation Command: None
   - Command: `cd "$(git rev-parse --show-toplevel)/app/server" && uv run pytest tests/ -v --tb=short`
...
4. **TypeScript Type Check**
   - Preparation Command: None
   - Command: `cd "$(git rev-parse --show-toplevel)/app/client" && bun tsc --noEmit`
...
5. **Frontend Build**
   - Preparation Command: None
   - Command: `cd "$(git rev-parse --show-toplevel)/app/client" && bun run build`
...
[
  {
    "test_name": "frontend_build",
    "passed": false,
    "execution_command": "cd \"$(git rev-parse --show-toplevel)/app/client\" && bun run build",
  ...
  },
  {
    "test_name": "all_backend_tests",
    "passed": true,
    "execution_command": "cd \"$(git rev-parse --show-toplevel)/app/server\" && uv run pytest tests/ -v --tb=short",
    "test_purpose": "Validates all backend functionality including file processing, SQL security, LLM integration, and API endpoints"
  }
]
```