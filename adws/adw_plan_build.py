...
- Claude Code CLI: Installed and authenticated (uv tool install claude-code)
...
def check_env_vars(logger: Optional[logging.Logger] = None) -> None:
    ...
    Note: ANTHROPIC_API_KEY is not required when using Claude Code CLI directly.
    Claude Code handles authentication automatically.
    """
    # Claude Code CLI is the only real requirement
    # It can be called as 'claude' if installed via 'uv tool install claude-code'
    error_msg = None
    try:
        result = subprocess.run(["claude", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode != 0:
            error_msg = "Error: Claude Code CLI is not installed. Install with: uv tool install claude-code"
...
def classify_issue(
    ...
    if issue_command == "0":
        logger.warning(
            f"Could not classify issue #{issue.number} (title: '{issue.title}'). "
            f"Body: '{issue.body[:100] if issue.body else 'empty'}'. "
            f"Defaulting to /feature classification."
        )
        # Default to feature if unclassifiable
        return "/feature", None

    if issue_command not in ["/chore", "/bug", "/feature"]:
        logger.warning(f"Invalid classification returned: {issue_response.output}. Defaulting to /feature.")
        # Default to feature if invalid
        return "/feature", None
