...
@app.post("/api/generate-query", response_model=QueryGenerationResponse)
async def generate_query_endpoint(request: QueryGenerationRequest) -> QueryGenerationResponse:
    """Generate a natural language query based on current database schema"""
    try:
        # Get database schema
        schema_info = get_database_schema()
        ...
        response = QueryGenerationResponse(query=query)
