...
async def generate_query_endpoint(request: GenerateQueryRequest) -> GenerateQueryResponse:
    ...
    try:
        ...
        if not schema_info.get('tables') or len(schema_info['tables']) == 0:
            raise Exception("No tables found in database. Please upload data first.")

        # Generate natural language query
        query_text = generate_natural_language_query(schema_info)

        response = GenerateQueryResponse(query=query_text)
        logger.info(f"[SUCCESS] Query generated: {query_text}")
        return response
    except Exception as e:
