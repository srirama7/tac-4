...
from core.data_models import (
    ...
    TableSchema,
    ColumnInfo,
    NLQueryGeneratorRequest,
    NLQueryGeneratorResponse
)
...
@app.post("/api/generate-nl-query", response_model=NLQueryGeneratorResponse)
async def generate_nl_query_endpoint(request: NLQueryGeneratorRequest) -> NLQueryGeneratorResponse:
    ...
    try:
        ...
        if table_count == 0:
            return NLQueryGeneratorResponse(
                generated_query="",
                table_count=0,
        ...
        generated_query = generate_natural_language_query(schema_info, llm_provider)

        response = NLQueryGeneratorResponse(
            generated_query=generated_query,
            table_count=table_count
        )
        logger.info(f"[SUCCESS] Generated NL query: {generated_query}")
        return response
    except Exception as e:
        logger.error(f"[ERROR] NL query generation failed: {str(e)}")
        logger.error(f"[ERROR] Full traceback:\n{traceback.format_exc()}")
        return NLQueryGeneratorResponse(
            generated_query="",
            table_count=0,
