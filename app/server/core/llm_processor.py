...
def generate_sql(request: QueryRequest, schema_info: Dict[str, Any]) -> str:
    ...
    # Check API key availability first (Gemini priority for SQL operations)
    if gemini_key:
...
Generate an interesting natural language question that a user could ask about this data. The question should:
...
- Be limited to a maximum of two sentences
- Vary in type (aggregations, filters, comparisons, trends, joins, etc.)
...
def generate_natural_language_query_with_gemini(schema_info: Dict[str, Any]) -> str:
    """
    Generate a natural language query suggestion using Google Gemini API
    """
    try:
        # Get API key from environment
        api_key = os.environ.get("GEMINI_API_KEY")
...
Question:"""
        ...
        raise Exception(f"Error generating query with Gemini: {str(e)}")

def generate_natural_language_query(llm_provider: str, schema_info: Dict[str, Any]) -> str:
    """
    Route to appropriate LLM provider for query generation.
    Priority: 1) Gemini API key exists, 2) OpenAI API key exists, 3) Anthropic API key exists, 4) llm_provider parameter
    """
    gemini_key = os.environ.get("GEMINI_API_KEY")
    openai_key = os.environ.get("OPENAI_API_KEY")
    anthropic_key = os.environ.get("ANTHROPIC_API_KEY")
    ...
    # Fall back to llm_provider preference if both keys available or neither available
