...
Requirements:
- The query should be creative and insightful, not generic
- Vary the query type: aggregations (total, average, count), filters (find, show, get), comparisons, joins (if multiple tables), time-based queries (if date columns exist)
- Make the query relevant to the actual columns and data types present
- Keep it to TWO SENTENCES MAXIMUM
- Return ONLY the natural language query text, no explanations or additional context
- Do NOT return SQL, return natural language that a human would ask
...
Natural Language Query:"""
        ...
        response = client.chat.completions.create(
            model="gpt-4.1-mini",
            messages=[
                {"role": "system", "content": "You are a creative assistant that generates interesting natural language database queries."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.9,
            max_tokens=200
        ...
        # Ensure it's two sentences max (split by '. ' and take first two)
...
Natural Language Query:"""
        ...
        # Call Anthropic API with higher temperature for variety
        response = client.messages.create(
            model="claude-3-haiku-20240307",
            max_tokens=200,
            temperature=0.9,
...
def generate_random_query(schema_info: Dict[str, Any]) -> str:
    ...
    if openai_key:
        ...
        raise ValueError("No LLM API key configured. Please set OPENAI_API_KEY or ANTHROPIC_API_KEY environment variable.")